# Contributing to Buckshot Roulette Randomizer
...
> If you want to ask a question, we assume that you have read the available [Documentation](https://git.graesberg.com/buckshot-roulette-randomizer/blob/main/README.md).
...
We will then try to answer you question as soon as possible.
...
- Determine if your bug is really a bug and not an error on your side e.g. using incompatible environment components/versions (Make sure that you have read the [documentation](https://git.graesberg.com/buckshot-roulette-randomizer/blob/main/README.md). If you are looking for support, you might want to check [this section](#i-have-a-question)).
...
- Open an [Issue](https://git.graesberg.com/buckshot-roulette-randomizer/issues/new). (Since we can't be sure at this point whether it is a bug or not, we ask you not to talk about a bug yet and not to label the issue.)
...
- Make sure that you are using the latest version.
- Read the [documentation](https://git.graesberg.com/buckshot-roulette-randomizer/blob/main/README.md) carefully and find out if the functionality is already covered, maybe by an individual configuration.
...
- Use arrow funtions (```const _ = () => {}```) rather than regular ```function``` declarations
  - If your component only returns JSX, omit the ```{}``` for ```()```
    ...
    const RightComponent = () => (//✅ Do this instead
      <>                          //✅ Do this instead
